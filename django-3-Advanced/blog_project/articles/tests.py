...
class ArticleAuthenticationTests(TestCase):
    ...
    def test_protected_views_redirect_anonymous_users(self):
        """
        Test 1: Vérification que les utilisateurs non connectés
        sont redirigés
        """
        protected_urls = {
        ...
        for name, url in protected_urls.items():
            ...
            self.assertEqual(
                ...
                302,
                (
                    f"\nTest échoué : La vue '{name}' "
                    "n'a pas redirigé l'utilisateur anonyme"
                )
            )
            print(
                (
                    f"✓ La vue '{name}' "
                    "redirige bien les utilisateurs non connectés"
                )
            )

    def test_protected_views_accessible_to_logged_users(self):
        """
        Test 2: Vérification que les utilisateurs connectés
        peuvent accéder aux vues protégées
        """
        ...
        for name, url in protected_urls.items():
            ...
            self.assertEqual(
                ...
                200,
                (
                    f"\nTest échoué : L'utilisateur connecté "
                    f"ne peut pas accéder à '{name}'"
                )
            )
            print(
                f"✓ La vue '{name}' est accessible aux utilisateurs connectés"
            )

    def test_register_view_forbidden_for_authenticated_users(self):
        """
        Test 3: Vérification que la page d'inscription est
        inaccessible aux utilisateurs connectés
        """
        self.client.login(username='testuser', password='testpass123')
        ...
        self.assertEqual(
            ...
            302,
            (
                "\nTest échoué : Un utilisateur connecté "
                "peut accéder à la page d'inscription"
            )
        )
        print(
            "✓ La page d'inscription est bien "
            "inaccessible aux utilisateurs connectés"
        )

    def test_duplicate_favourite_forbidden(self):
        """
        Test 4: Vérification de l'impossibilité d'ajouter
        deux fois le même article en favoris
        """
        self.client.login(username='testuser', password='testpass123')

        # Premier ajout en favoris
        response1 = self.client.post(
            reverse('add_favourite', kwargs={'pk': self.article.pk})
        )
        self.assertEqual(
