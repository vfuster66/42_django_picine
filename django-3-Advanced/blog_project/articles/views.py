...
class UserLoginView(LoginView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        logger.error(f"GET request to login view from: {referer}")
...
class RegisterView(CreateView):
    ...
    def form_valid(self, form):
        ...
        logger.error(f"Registration form valid for user: {username}")
...
class PublishArticleView(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        logger.error(
            f"Publishing article: {form.cleaned_data.get('title')} by "
            f"{self.request.user}"
        )
...
class AddToFavouriteView(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        logger.error(
            f"Adding article {self.kwargs['pk']} to favourites for user "
            f"{self.request.user}"
        )
        return super().form_valid(form)
